                    ...
                    withSonarQubeEnv('sonarqubeserver') {
                        script {
                            ...
                            withSonarQubeEnv(credentialsId: 'sonarqube') {
                                bat """
                                    ${scannerHome}\\SonarScanner.MSBuild.exe begin /k:"FiberGIS_CatalogoApi" /d:sonar.verbose=true
        ...
        stage('Run Docker container') {
            steps {
                sshagent(['SSH_Server_135_geouser']) {
                    sh '''
                        ssh geouser@192.168.1.135 " 
                            docker run -v /home/geouser/pasaje/wwwroot:/app/wwwroot -d --restart=always -p 44322:80 --name fgcatalogoapi fgcatalogoapi:qa
                        "
                    '''
                }
            }
